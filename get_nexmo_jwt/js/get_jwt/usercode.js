module.exports = (data) => {
    ...
    for (i = 0; i < data.length; i += 3) {
        ...
        binData = (data[i] & 0xFF) << 16 |     // FF.00.00
                  (data[i + 1] & 0xFF) << 8 |  // 00.FF.00
                  (data[i + 2] & 0xFF);        // 00.00.FF
        ...
        result += base64Alphabet[(binData & 0xFC0000) >>> 18] +                   //11111100.00000000.00000000 = 0xFC0000 = 16515072
                  ...
                  base64Alphabet[( i + 3 >= data.length && (data.length << 1) % 3 === 2 ? 64 :
                                     (binData & 0x000FC0) >>> 6 )] +              //00000000.00001111.11000000 = 0x000FC0 = 4032
                  base64Alphabet[( i + 3 >= data.length && (data.length << 1) % 3 ? 64 :
